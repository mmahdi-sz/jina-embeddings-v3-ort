...
fn main() -> Result<()> {
    ...
    let zwnj_test_cases = [
        ("فعل حال (می‌روم)", "من به دانشگاه می‌روم", "من به دانشگاه می روم", "من به دانشگاه میروم"),
        ("جمع نام‌ها (کتاب‌ها)", "تمام کتاب‌ها را خواندم", "تمام کتاب ها را خواندم", "تمام کتابها را خواندم"),
        ("ترکیب اسمی (دانش‌آموزان)", "دانش‌آموزان در کلاس درس حاضر شدند", "دانش آموزان در کلاس درس حاضر شدند", "دانشاموزان در کلاس درس حاضر شدند"),
        ("شناسه ملکی (خانه‌ام)", "این کلید خانه‌ام است", "این کلید خانه ام است", "این کلید خانهام است"),
        ("قید مرکب (دست‌کم)", "دست‌کم ده نفر در جلسه شرکت کردند", "دست کم ده نفر در جلسه شرکت کردند", "دستکم ده نفر در جلسه شرکت کردند"),
        ("صفت منفی (بی‌نقص)", "طراحی این سیستم کاملاً بی‌نقص و زیباست", "طراحی این سیستم کاملاً بی نقص و زیباست", "طراحی این سیستم کاملاً بینقص و زیباست"),
        ("اسم جمع محاوره‌ای (بچه‌ها)", "بچه‌ها در حیاط بازی می‌کنند", "بچه ها در حیاط بازی می‌کنند", "بچهها در حیاط بازی می‌کنند"),
        ("صفت برتر (فوق‌العاده)", "این یک نتیجه فوق‌العاده در بنچمارک است", "این یک نتیجه فوق العاده در بنچمارک است", "این یک نتیجه فوقالعاده در بنچمارک است"),
        ("ترکیب فعلی (پیش‌بینی)", "پیش‌بینی وضعیت آب و هوا در روزهای آینده", "پیش بینی وضعیت آب و هوا در روزهای آینده", "پیشبینی وضعیت آب و هوا در روزهای آینده"),
        ("قید زمانی (هم‌اکنون)", "برنامه هم‌اکنون به صورت زنده در حال پخش است", "برنامه هم اکنون به صورت زنده در حال پخش است", "برنامه هماکنون به صورت زنده در حال پخش است"),
    ];

    let mut zwnj_results = Vec::new();

    println!("  {:<25} | {:<12} | {:<12} | {:<8}", "Concept", "ZWNJ vs Space", "ZWNJ vs Merge", "Status");
    println!("  {}", "-".repeat(65));
    ...
    for (concept, zwnj, space, merged) in &zwnj_test_cases {
        ...
        println!("  {:<25} | {:>10.4}   | {:>10.4}   | {:<8}", concept, cos_space, cos_merged, if pass { "PASS" } else { "WARN" });
    ...
    let char_variants = [
        ("یای فارسی vs يای عربی (ی vs ي)", "پایگاه داده توزیع‌شده با کارایی بالا", "پايگاه داده توزيع‌شده با کارايي بالا"),
        ("کاف فارسی vs كاف عربی (ک vs ك)", "کمک به کودکان در مناطق محروم کشور", "كمك به كودكان در مناطق محروم كشور"),
        ("تای تانیث vs های فارسی (ة vs ه)", "توسعه اقتصادی و پیشرفت پایدار جامعه", "توسعة اقتصادی و پیشرفت پایدار جامعة"),
        ("اعداد فارسی vs اعداد انگلیسی", "فروش ۳۲۵۰۰ عدد کالا در سال ۱۴۰۳", "فروش 32500 عدد کالا در سال 1403"),
        ("علامت سوال فارسی vs انگلیسی (؟ vs ?)", "چگونه یک وب‌سرور پرسرعت بسازیم؟", "چگونه یک وب‌سرور پرسرعت بسازیم?"),
        ("گیومه فارسی vs انگلیسی («» vs \"\")", "کتاب «هنر شفاف اندیشیدن» را مطالعه کنید", "کتاب \"هنر شفاف اندیشیدن\" را مطالعه کنید"),
    ];

    let mut char_results = Vec::new();
    println!("  {:<42} | {:<12} | {:<8}", "Character Variant", "Cosine Sim", "Status");
    println!("  {}", "-".repeat(68));
    ...
    for (desc, standard, variant) in &char_variants {
        ...
        println!("  {:<42} | {:>10.4}   | {:<8}", desc, cos, if pass { "PASS" } else { "WARN" });
    ...
    println!("  {:<32} | {:<8} | {:<10} | {:<8}", "Input Type", "Dim", "L2 Norm", "Status");
    println!("  {}", "-".repeat(65));
    ...
    for (name, text) in &extreme_inputs {
        ...
        println!("  {:<32} | {:>6}   | {:>10.6} | {:<8}", name, emb.len(), norm, if pass { "PASS" } else { "FAIL" });
    ...
    println!("  {:<18} | {:<12} | {:<8} | {:<10} | {:<12}", "Scale (Repeat)", "Char Count", "Dim", "L2 Norm", "Latency");
    println!("  {}", "-".repeat(70));
    ...
    for scale in &truncation_scales {
        ...
        println!("  {:<18} | {:>10}   | {:>6}   | {:>10.6} | {:>10.2?}", format!("{}x (~{} tok)", scale, est_tokens), long_text.len(), emb.len(), norm, duration);
    ...
    let out_path = PathBuf::from("/mnt/data/mahdidev/onnx/python-ref/suite2_edge_case_results.json");
